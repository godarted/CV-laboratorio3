...
var rbMas = document.getElementById("txtrbMas");
    ...
    db.once("value", function (snapshot) {
        if (snapshot.exists()) {
            snapshot.forEach(function (data) {
                ...
                cboDistrito.addEventListener(options);
            });
        }
    });
}
//creamos un procedimiento para mostrar losdatos de la tabla
function Mostrar() {
    // declaramos una variables para contar el numero de filas
    var i = 0;
    //seleccionamos el tbody de la tabla donde voy a mostrar la informacion
...
function Registrar() {
    ...
    // llamamos a la funcion para registrar del FIrebase
    // writeUserData(nomb, ape, cor);
...
function Eliminar(codigo) {
    //preguntamos si quiere eliminar el registro
    var result = confirm("¿Estas seguro de que quieres eliminar el registro?");
